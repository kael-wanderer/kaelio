...
function getRecentFiles(): RecentEntry[] {
  try { return JSON.parse(localStorage.getItem("kaelio-web-recent") || "[]"); }
  catch { return []; }
}
...
function addRecentFile(name: string, content: string) {
  ...
  localStorage.setItem("kaelio-web-recent", JSON.stringify(recent));
  renderRecentFiles();
}
...
let currentThemeMode: ThemeMode = (localStorage.getItem("kaelio-theme") as ThemeMode) || "auto";
...
function onContentChange(view: EditorView) {
  ...
  debounceTimer = setTimeout(() => {
    ...
    localStorage.setItem("kaelio-web-autosave", content);
    localStorage.setItem("kaelio-web-autosave-name", currentFileName || "Untitled");
  }, 300);
...
function dismissBanner() {
  ...
  localStorage.setItem("kaelio-web-banner-dismissed", "1");
}

// --- Sample content ---

const SAMPLE_CONTENT = `# Kaelio -- Markdown Editor for the AI Era

**Kaelio** is a fast, lightweight markdown editor that picks up where [MacDown](https://macdown.uranusjr.com/) left off (abandoned since 2020). No vault, no config, no signup -- just open and write.

Built for people who work with AI daily and need a clean way to read, edit, and share markdown.

## Why Kaelio?

| | Kaelio | MacDown | Typora | VS Code |
|---|---|---|---|---|
...
Your work is auto-saved to your browser. For file associations, PDF export, and auto-updates, [download the desktop app](https://github.com/kael-wanderer/kaelio/releases/latest).

---

**Open source** -- [github.com/kael-wanderer/kaelio](https://github.com/kael-wanderer/kaelio)
`;
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const savedContent = localStorage.getItem("kaelio-web-autosave");
  const savedName = localStorage.getItem("kaelio-web-autosave-name");
  ...
  // Banner
  if (localStorage.getItem("kaelio-web-banner-dismissed") === "1") {
    document.getElementById("banner")?.remove();
  }
