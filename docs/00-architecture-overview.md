...
Kaelio is a Tauri 2 desktop markdown editor with live preview, Mermaid diagrams, KaTeX math, and word counting. The frontend runs vanilla TypeScript via Vite; the backend runs Rust for file I/O and PDF export.
...
| Entry | File | Role |
|-------|------|------|
| Rust main | `src-tauri/src/main.rs` | Launches `kaelio_lib::run()` |
