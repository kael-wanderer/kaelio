...
fn kaelio_home() -> Result<PathBuf, String> {
    ...
    let new_dir = PathBuf::from(&home).join(".kaelio");
    let old_dir = PathBuf::from(&home).join(".mx");
...
#[tauri::command]
fn save_binary_base64(path: String, data_base64: String) -> Result<(), String> {
...
fn get_recovery_dir() -> Result<PathBuf, String> {
    let dir = kaelio_home()?.join("recovery");
...
fn sidecar_path_env(app: &tauri::AppHandle, base_path: &str) -> String {
    ...
    for base in ["pandoc", "typst"] {
        if let Some(path) = find_bundled_binary(app, base) {
...
fn pandoc_command(app: &tauri::AppHandle) -> Command {
    if let Some(path) = find_bundled_binary(app, "pandoc") {
        Command::new(path)
    } else {
        Command::new("pandoc")
    }
...
fn export_pdf_blocking(markdown_content: String, output_path: String, source_format: String, app: tauri::AppHandle) -> Result<String, String> {
    ...
    for engine in engines {
        ...
        let mut child = command
            ...
            .spawn()
            .map_err(|e| format!("Pandoc not found in app bundle or system PATH. ({})", e))?;
    ...
    std::thread::spawn(move || {
        let result = (|| -> Result<String, String> {
            ...
            let mut child = command
                ...
                .stderr(std::process::Stdio::from(stderr_out))
                .spawn()
                .map_err(|e| format!("Failed to run bundled or system pandoc: {}", e))?;
...
fn load_custom_css() -> Result<String, String> {
    let path = kaelio_home()?.join("preview.css");
    if path.exists() {
...
/// Save a snapshot of a file to ~/.kaelio/snapshots/
#[tauri::command]
    ...
    tauri::Builder::default()
        ...
        .menu(|handle| {
            ...
            let file_menu = Submenu::with_items(handle, "File", true, &[
                ...
                &MenuItem::with_id(handle, "file.export-html", "Export HTML...", true, None::<&str>)?,
